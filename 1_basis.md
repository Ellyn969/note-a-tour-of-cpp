...
* 初始化的讲究
    ...
    * 可以不显式初始化的情况：**`auto`**
        * 变量类型可以自动推断
            ...
            auto a=3
            ```
        * `auto`对变量进行推导时，默认表现为 **“按值拷贝”**，因此会忽略引用（&）和顶层 const。
            * 即：除非显式指定 `auto&` 或 `const auto&`，否则变量会被按值拷贝
* **整数除法会截断小数部分**
    ```cpp
    int x = 5 / 2; // x == 2
    ```
...
* 指针、引用和const:
    ...
    * `const int& r`: 指向常量int的引用：r引用的**内容**不可修改(指向的地址本来就不能修改)
        * `const int& ref = 10;`: 合法。常量左值引用可以绑定到临时对象（`10`），延长其生命周期
        * 然而，`int& ref = 10;` 不合法 --> 需要`int&& ref = 10;`（右值引用）或`const int& ref = 10;`
...
* 数组 = 连续内存块
    ...
    * 无边界检查：越界访问后果严重，UB
* C++ 标准规定 **`sizeof(char)` 始终为 1**。其他类型的大小（如 int）则取决于具体实现。