...
* **翻译单元**：一个可以单独编译的`.cpp`文件 + 它所包含的所有**头文件**，经过预处理器处理后的结果
...
* 对于大对象的返回：
    * 现代C++中通常**直接返回大对象**，而不是返回引用：
        ...
        ```
    * **RVO (Return Value Optimization)**: 编译器自动优化此类复制：**省略复制优化** 
        * 并非直接复制
    * 如果不能进行优化：会给大对象提供**移动构造方法**（能以非常小的cost将大对象传递到函数之外）
        * **以避免昂贵的深拷贝**
* **返回类型推导**：(自动推导返回类型) 
    ```cpp
    ...
    ```
    * **不要过度使用**
        * 必须看到 return 语句才能推导，这意味着实现必须暴露给调用者（通常在头文件中），且用户无法一眼看出返回类型。
* **返回类型后置**：
    ```cpp
...
* **结构化绑定**：（返回多个值）
    ...
    cout << n << " " << v;
    ```
    * **结构化绑定**：`auto [var1, var2, ...] = expr;`
        * expr返回一个聚合类型（如struct、pair、tuple等）
        * var1, var2分别绑定到expr的各个成员上
        * **实际是一种拷贝行为**
            * 即：无法通过该方法修改expr的成员
    * 用途示例：遍历map:
        ```cpp
