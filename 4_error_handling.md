...
* `throw`语句
    ...
    * ```cpp
        ...
        * `catch(out_of_range& e)`：**通过引用捕获异常对象**，避免拷贝开销
        * `cerr`：标准错误输出流
        * `e.what()`
        * **不要过多使用`try`语句**
...
# 错误处理的其他方法
* 设计 **【异常】机制**目的：提供一个**将错误检测和错误处理分离**的方法
* 异常:
    * 用于报告在当前函数内部无法处理的错误
    * **与构造函数和析构函数配合使用**（即**RAII技术**），确保资源的正确管
* **【如何理解错误码和异常的区别】**
    * 错误码：如果你忘记检查返回值，程序会继续运行 --> BUG
        * 因此，代码中会包含大量`if`检查，代码臃肿，代码逻辑被打乱
    * 异常：程序遇到错误时，**自动跳转**到异常处理代码（`try-catch`），如果没有对应处理，程序会终止 --> **【不可能忽略此类错误】**
        * **错误处理代码和正常代码分离**

## 1. 抛出异常throw exceptions
* 最通用的方法
* 标准库提供了一套异常类（如`std::out_of_range`、`std::bad_alloc`等）
    * 但也可以定义**自定义异常类**
* 不需要手动检查返回值
...
* 建议在以下**不得不由上层处理 或 语法限制**的情况下使用：
    * 失败是罕见且不可预期的（**意外情况**）
        * e.g., 内存耗尽
        * 当`new`无法分配内存时，会抛出`std::bad_alloc`异常（通常不需要检查`new`的返回值是否为空）
    * **错误无法在当前函数内处理，需要层层上报**
        * e.g., 深层嵌套的函数调用
...
* *Debug和Release模式*
    * **Debug模式**：用于开发和调试
    * **Release模式**：开发已经完成，面向用户：为了性能，编译器通常会完全删除所有的`assert`语句（定义`NDEBUG`宏）
* 在**运行时**断言某个条件必须满足
    * 处理绝不应该发生的事
    * 如果发生了，那就是有bug
