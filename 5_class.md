...
* 例子：`complex`类 (标准库的简化版本)
    ...
    * 简单操作，如赋值和`+=`，不应该以函数调用形式实现
        ...
        * 也可以在类外定义`inline`函数
    * `const`修饰符
        * in `double real() const { return re; } // 返回实部`
        * **类函数用const修饰表示：**
            * **该函数不能修改成员变量**（除非成员变量被声明为`mutable`）
        * 留意`const`的位置
...
* 析构函数：
    ...
    * `delete`：释放一个独立对象
    * `delete[]`：释放一个数组
    * C++ 标准规定 delete 空指针是安全的操作，无需检查 if (p)
...
* **将使用者和类的实现细节完全隔离开来**
    ...
    * --> **放弃纯局部变量**
    * **没有构造函数**：不需要初始化数据
    * **析构函数必须被定义为`virtual`：虚析构函数**
        * 否则：通过基类指针删除派生类对象时，派生类的析构函数不会被调用，导致*资源泄露*
        * 不一定是纯虚
* **虚函数**
    * 虚函数表`vtbl`
...
* 示例：
    ...
    // 多态引用示例
    void process_shape(Shape& s) { 
        // 转换引用：如果不符合，会抛出`std::bad_cast`异常
        try {
